...
void Graph::add_meters(int frm, int to, double meter) {
    this->meters.insert({std::make_pair(frm, to), meter});
}

void Graph::add_seconds(int frm, int to, double second) {
    this->seconds.insert({std::make_pair(frm, to), second});
}
...
void Graph::route_planning_details(int start_node, int end_node) {
    ...
    while (cause != start) {
        ...
        (meter).push_back(this->meters[std::make_pair(cause->prev->id, cause->id)]);
        (second).push_back(this->seconds[std::make_pair(cause->prev->id, cause->id)]);
        cause = cause->prev;
    }
    ...
    std::reverse((meter).begin(), (meter).end());
    std::reverse((second).begin(), (second).end());

    std::ofstream result;
    result.open("./result/optimal_path.csv");

    result << "longitude" << "," << "latitude" << "," << "distance (m)" << "," << "time (sec)" << "\n";
    for (int i = 0; i < longitude.size(); i++) {
        result << longitude[i] << "," << latitude[i] << "," << meter[i] << "," << second[i] << "\n";
    }
